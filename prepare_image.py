...
def prepare_image(image: np.ndarray, mask: np.ndarray, cut: np.ndarray):
    ...
    merged = cv.copyMakeBorder(merged, tile_size - merged.shape[0] % tile_size, 0,
                            tile_size - merged.shape[1] % tile_size, 0, cv.BORDER_CONSTANT, value=[0, 0, 0])

    mask = cv.copyMakeBorder(mask, tile_size - mask.shape[0] % tile_size, 0,
                            tile_size - mask.shape[1] % tile_size, 0, cv.BORDER_CONSTANT, value=[0, 0, 0])
    ...
    data = [means.flatten(), _vars.flatten(), moments_0.flatten(),
    moments_1.flatten(), moments_2.flatten(), moments_3.flatten(), moments_4.flatten(), truth.flatten()]    
    knn_data_frame = pd.DataFrame(data, index=['mean', 'var', 'moment 0', 'moment 1', 'moment 2', 'moment 3', 'moment 4', 'truth']).transpose()
